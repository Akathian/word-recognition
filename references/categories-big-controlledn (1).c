...
char *argv[] ;
{
  ...
  int	nFeatures, nCategories, nMembers, minOn, maxOn, numOn ;
  float minProbOn, maxProbOn, probOn, minProbDistort, maxProbDistort, probDistort ;

  if (argc != 13) {
    printf("Usage: %s <nFeatures> <nCategories> <nMembers> <minProbOn> <maxProbOn> <minDiff> <minProbDistort> <maxProbDistort> <sparseFormat> <seed> <minOn> <maxOn>\n", argv[0]) ;
    exit(0) ;
  }
  ...
  for (c = 0 ; c < nCategories ; c++) {
    ...
    */
	...
	if(numOn > maxOn) new = FALSE;
	if(numOn < minOn) new = FALSE;      
